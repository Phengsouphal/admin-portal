...
const onSubmit = () => {
  ...
  discountStore.onEditDiscount(form, id)
}
...
onMounted(async () => {
  const id = route.params.id as string
  const res = await discountStore.getDiscountById(id)
  if (res) {
    const discount = res.discount as IDiscountListData
    form.productIds = discount.productIds
    form.name = discount.name
    form.description = discount.description
    form.type = discount.type
    form.value = discount.value
    form.valueType = discount.valueType
    form.isNoMinPurchaseAmount = '' as '' | 'NO' | 'YES'
    // form.minPurchaseAmount = discount.form.applyTo
    form.publishTo = discount.publishTo
    form.applyTo = discount.applyTo

    form.maxUseLimit = discount.maxUseLimit
    form.usageLimit = discount.usageLimit
    form.startAt = dayjs(discount?.startAt)?.format('YYYY-MM-DD')
    form.endAt = dayjs(discount?.endAt)?.format('YYYY-MM-DD')
  }
})
</script>
