<template>
  <div class="w-full h-full px-6 py-5">
    <McTitle :title="$t('button.addProduct')"> </McTitle>
    <a-form
      ref="formRef"
      :model="form"
      :layout="'vertical'"
    ...
    >
      <McCardHeader :title="$t('product.gallery')">
        <div class="w-full py-6 overflow-x-scroll relative">
          <McUpload :is-error="isUploadError" @on-uploaded="onUploaded" @on-delete="onDelete">
          </McUpload>
        </div>
      </McCardHeader>
      ...
      <McCardHeader :title="$t('product.pricing')">
        <div class="w-full py-2">
          <a-form-item
            ...
            field="price"
            :label="$t('field.price')"
            :rules="[{ required: true, message: $t('field.priceIsRequired') }]"
          >
            <InputNumber
              v-model="form.price"
              :hide-button="true"
        ...
        >
          ...
          <div class="w-full py-2 gap-y-2" v-if="form.variants.length > 0">
            <p class="text-main font-semibold my-2 text-left">
              {{ form.variants.length > 1 ? 'Added Products' : 'Added Product' }}
      ...
      <McCardHeader :title="$t('product.category')">
        <template #headerRight>
          <a-button v-if="cateList.length > 0" type="text" class="mr-4" @click="onShowCreateCate">
        ...
        <div class="w-full flex justify-start mb-3">
          <a-form-item
            ...
            :required="true"
            :rules="[{ required: true, message: $t('field.categoryIsRequired') }]"
          >
            <a-radio-group
            ...
            <div v-else class="w-full py-5 flex flex-col justify-center items-center">
              <IconCategoryEmptyColor> </IconCategoryEmptyColor>
              <p class="text-sm font-medium text-[#111827] mt-2">{{ $t('category.noCategory') }}</p>
              <p class="text-sm font-normal text-[#6B7280] mt-1">
                {{ $t('category.clickAddCategory') }}
              </p>
              <a-button
                type="primary"
                class="!mt-4 !h-[38px] !rounded-md"
                @click="onShowCreateCate"
              >
                <template #icon>
                  <IconPlus></IconPlus>
                </template>
                <!-- Use the default slot to avoid extra spaces -->
                <template #default>{{ $t('button.addCategory') }}</template>
              </a-button>
            </div>
          </a-form-item>
        </div>
...
import { v4 } from 'uuid'
import { DiscountEnum } from '~/constants/common'
...
const form = reactive({
  ...
  price: null,
  discount: '',
  discountType: 'AMOUNT' as DiscountEnum,
  categoryId: '',
  collectionIds: [],
  imgIds: [] as any[],
  variants: [] as any[]
...
const onSubmit = async () => {
  ...
  await productStore.onCreateProduct(
    form.categoryId,
    form.collectionIds,
    form.description,
    form.discount,
...
:deep(.cate-form-item .arco-form-item-label-col) {
  display: none;
}
</style>
