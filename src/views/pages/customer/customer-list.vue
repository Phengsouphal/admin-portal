<template>
  <div class="w-full h-full px-6 pt-5">
    <McTitle :title="$t('customer.title')"> </McTitle>
    <McCard
      v-if="list.length === 0 && !loading"
      :hasBtnAdd="false"
      :title="$t('customer.noCustomer')"
      :subTitle="$t('customer.youDontHaveCustomer')"
    >
      <template #icon>
      ...
      <div class="w-full">
        <div class="w-full py-4 flex justify-between items-center">
          <p class="text-lg font-medium text-[#111827]">
            {{ $t('customer.allCustomer') }}({{ customerStore.customerMeta?.itemCount }})
          </p>
        ...
        <div v-if="!loading" class="w-full py-2">
          ...
          >
            ...
            <template #date="{ record }">
              {{ dayjs(record?.customer?.createdAt).format('DD/MM/YYYY') }}
            </template>
            ...
            <template #action="{ record }">
              <div class="flex justify-start items-center gap-x-2">
                <a-button
                  type="text"
                  class="!px-0"
                  @click="$router.push('/customer-detail/' + record._id)"
                >
            ...
            <template #pagination-left>
              <div class="w-full text-black text-left">
                {{
                  $t('table.showResult', {
                    total: customerStore.customerMeta?.totalItems,
                    from: 1,
                    to: customerStore.customerMeta?.itemCount
                  })
                }}
              </div>
            </template>
