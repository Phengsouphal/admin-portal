<template>
  <div class="w-full h-full px-6 pt-5">
    <McTitle
      :title="$t('promotion.voucher')"
      :has-btn-add="list.length > 0"
      :btn-txt="$t('button.createVoucher')"
      @on-click="$router.push('/' + RouteName.ADD_VOUCHER)"
    >
    </McTitle>
    <McCard
      v-if="list.length == 0 && !loading"
      :title="$t('voucher.manageVoucher')"
      :subTitle="$t('voucher.clickToVoucher')"
      :btn-txt="$t('button.createVoucher')"
      @onClick=""
    >
      <template #icon>
    ...
    <McCardHeader v-else>
      <div class="w-full">
        <div class="w-full flex justify-between items-center mt-4 border-b border-[#E5E6EB]">
          ...
          <a-dropdown @select="handleDeleteSelect" position="br">
        ...
        <div class="w-full py-2" v-if="!loading">
          <a-table
            ...
            :show-header="false"
            :pagination="{
              total: voucherStore.voucherMeta?.totalItems,
              current: voucherStore.voucherMeta?.currentPage
            }"
            :columns="columns"
            :data="list"
            :scroll="{ x: 700 }"
          ...
          >
              ...
              <div class="flex justify-start items-start">
                <div class="mr-5">
                  <a-typography-paragraph
                    class="text-base !mb-0 flex font-semibold text-black text-opacity-70"
                    copyable
                  >
                    {{ record.code }}
                    <template #copy-icon>
                  ...
                  <p class="text-sm font-medium text-[#6B7280] text-opacity-70">
                    {{ record.name }}
                  </p>
                </div>
            ...
            <template #type="{ record }">
              <div>
                <p class="text-base font-semibold text-black text-opacity-70">Amount off order</p>
                <p class="text-sm font-medium text-[#6B7280] text-opacity-70">
                  {{ $t('common.code') }}
                </p>
              </div>
            ...
            <template #action="{ record }">
              <div class="flex justify-end items-center gap-x-4">
                <a-button
                  type="text"
                  class="!p-0"
                  @click.stop="$router.push(`/${RouteName.EDIT_VOUCHER}/${record._id}`)"
                >
              ...
              <div class="w-full text-black text-left">
                {{
                  $t('table.showResult', {
                    total: voucherStore.voucherMeta?.totalItems,
                    from: 1,
                    to: voucherStore.voucherMeta?.itemCount
                  })
                }}
              </div>
            </template>
  ...
  <McDeleteModal
    :title="$t('voucher.deleteSelectedDiscountVoucher')"
    :sub-title="$t('voucher.deleteVoucherCannotBeUndo')"
    v-model:show="showSelectedDeleteModal"
    @on-cancel="showSelectedDeleteModal = false"
    @on-confirm="onConfirmDeleteSelected"
  ></McDeleteModal>
  <McDeleteModal
    :title="$t('voucher.deleteDiscountVoucher')"
...
const list = computed(() => voucherStore?.voucherList)
...
async function handleDeleteSelect() {
  showSelectedDeleteModal.value = true
}

async function onPageChange(v: any) {
  voucherStore.voucherQueryData.page = v
  loading.value = true
  // await fetchData()
  loading.value = false
}
...
const onConfirmDeleteSelected = async () => {
  for (const iterator of selectedKeys.value) {
    await voucherStore.deleteVoucherId(iterator)
  }
...
const onConfirmDelete = async () => {
  await voucherStore.deleteVoucherId(selectedId.value)
  showDeleteModal.value = false
  await fetchData()
}

async function fetchData() {
  await voucherStore.getVoucherList()
}
