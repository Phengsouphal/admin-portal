            ...
            >
              <div class="flex flex-col">
                ...
                >
                  ...
                  <template #help>
                    <div class="text-[#6B7280] text-opacity-70">{{ $t('voucher.applyToAllProduct') }}</div>
                  </template>
                </a-form-item>
          ...
          <McCardHeader :title="$t('voucher.activeDate')" class="pb-4">
            ...
            >
              <a-date-picker
                ...
                :show-now-btn="false"
                format="YYYY-MM-DD"
                :default-value="form.endAt"
                :model-value="form.endAt"
          ...
          <McCardHeader :title="$t('voucher.summary')">
            <div class="py-2 text-left">
              <template v-if="form.name !== '' || form.code != ''">
                <p class="text-[#111827] font-semibold mt-4 mb-2">{{ form.name }}</p>
...
const form = reactive({
  ...
  maxUseLimit: null as number | null,
...
const onSubmit = () => {
  ...
  voucherStore.onEditVoucher(form, id)
}
...
onMounted(async () => {
  ...
  if (res) {
    const voucher = res.voucher as IVoucherListData
    ...
    form.isNoMinPurchaseAmount = voucher.minPurchaseAmount === 100000 ? 'YES' : 'NO'
    form.minPurchaseAmount = voucher.minPurchaseAmount === 100000 ? null : voucher.minPurchaseAmount
    form.publishTo = voucher?.publishTo
    form.publishMode = voucher?.publishMode
    form.maxUseLimit = voucher?.maxUseLimit
    form.usageLimit = voucher?.usageLimit
    form.startAt = dayjs(voucher?.startAt)?.format('YYYY-MM-DD')
    form.endAt = dayjs(voucher?.endAt)?.format('YYYY-MM-DD')
    form.isEndDate = false
  }
})
</script>
